...
/* ════════════════════════════════════════════════════════════════
   FLY-FORWARD ENGINE — literal port of the turnkeyai.org mechanic.

   On load: wrap each existing top-level <section>/<header> as a
   <div class="panel"><div class="panel-inner">…original section…</div></div>,
   move them into a fixed #stage with perspective:1500px, append an
   invisible #scroll-driver (N * 122vh tall) for real page height, and
   flip body.engine-on so the natural-flow layout is hidden via CSS.

   A rAF camera loop reads window.scrollY → camTarget (0..N-1),
   eases camNow toward it (0.105/frame), and writes each panel's
   transform (translateZ = d * GAP), opacity (smoothstep gates),
   blur (8px on past, 4px on far ahead), and pointer-events (only
   the panel under the camera takes clicks).

   Knobs (TKAI defaults):
     PANEL_VH = 122   scroll runway per panel
     GAP      = 980   z-distance between panels
     EASE     = 0.105 camera follow per frame
     perspective = 1500px (set in CSS)
   ════════════════════════════════════════════════════════════════ */
function flyForwardEngine(){
  const REDUCE = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  // Knobs
  const PANEL_VH = 122;
  const GAP      = REDUCE ? 0 : 980;
  const EASE     = REDUCE ? 1 : 0.105;

  // ── 1. Discover the page's natural "shots" ──────────────────────
  // Top-level <section>/<header> children of body OR body>main.
  // Keep them in document order. Footer becomes the final panel.
  const candidates = [];
  document.body.childNodes.forEach(n => {
    if(n.nodeType !== 1) return;
    if(n.id === 'stage' || n.id === 'scroll-driver') return;
    if(n.classList.contains('energy-bg')) return;
    if(n.classList.contains('rail-fly') || n.classList.contains('scroll-hint-fly')) return;
    const tag = n.tagName.toLowerCase();
    if(tag === 'nav' || n.classList.contains('top-nav') || n.classList.contains('mobile-nav')) return;
    // Keep these as content shots
    if(tag === 'section' || tag === 'header' || tag === 'footer') candidates.push(n);
    else if(tag === 'main'){
      // Drill into <main>
      n.childNodes.forEach(c => {
        if(c.nodeType !== 1) return;
        const t = c.tagName.toLowerCase();
        if(t === 'section' || t === 'header' || t === 'footer') candidates.push(c);
      });
    }
  });
  if(candidates.length < 2) return;   // nothing to fly through

  // ── 2. Build the stage + driver + chrome ────────────────────────
  ...
  // Mark the top nav as the persistent chrome (don't hide it)
  const topNav = document.querySelector('body > .top-nav, body > nav.top-nav');
  if(topNav) topNav.classList.add('is-chrome');
  const mobileNav = document.querySelector('body > .mobile-nav');
  if(mobileNav) mobileNav.classList.add('is-chrome');

  // ── 3. Wrap each candidate into .panel > .panel-inner inside #stage ─
  const panels = [];
  candidates.forEach((src, i) => {
    ...
    inner.appendChild(src);     // move the original section in
  ...
  // ── 4. Scroll hint + chapter rail ───────────────────────────────
  const hint = document.createElement('div');
  hint.className = 'scroll-hint-fly';
  hint.innerHTML = '<span>Scroll to fly forward</span><span class="scroll-hint-fly-bar"><i></i></span>';
  document.body.appendChild(hint);

  const rail = document.createElement('nav');
  rail.className = 'rail-fly';
  rail.setAttribute('aria-label','Sections');
  panels.forEach((p, i) => {
    const btn = document.createElement('button');
    btn.className = 'rail-fly-step';
    btn.setAttribute('data-jump', String(i));
    btn.innerHTML = `<span class="tick"></span>${String(i+1).padStart(2,'0')}`;
    rail.appendChild(btn);
  });
  document.body.appendChild(rail);

  // ── 5. Flip engine-on AFTER move so layout doesn't jump for hidden frame
  document.body.classList.add('engine-on');

  // ── 6. Fit-to-viewport: scale .panel-inner down if content overflows ─
  function fitPanels(){
    ...
    for(let i=0;i<N;i++){
      ...
      const room = vh - parseFloat(cs.paddingTop) - parseFloat(cs.paddingBottom);
      const scale = (h > room && h > 0) ? Math.max(0.55, room / h) : 1;
  ...
  // ── 7. Camera state ─────────────────────────────────────────────
  let camTarget = 0, camNow = 0, maxScroll = 1;
  function recalcScroll(){ maxScroll = Math.max(1, document.documentElement.scrollHeight - window.innerHeight); }
  recalcScroll();
  function readScroll(){
  ...
  const railSteps = rail.querySelectorAll('.rail-fly-step');

  // ── 8. Render — places every panel for the current camera ──────
  function render(){
    for(let i=0;i<N;i++){
      const panel = panels[i];
      const d = camNow - i;
      const ad = Math.abs(d);
      const op = ss(-0.94,-0.30,d) * (1 - ss(0.18,0.88,d));
      if(op <= 0.004){
      ...
      const z = d * GAP;
      const ty = REDUCE ? 0 : d * -22;
      const blur = REDUCE ? 0 :
        clamp((d - 0.14) * 8, 0, 9) + clamp((-d - 0.55) * 4, 0, 2.5);
      panel.style.transform = `translate3d(0,${ty.toFixed(1)}px,0) translateZ(${z.toFixed(1)}px)`;
    ...
    const active = Math.round(camNow);
    railSteps.forEach((step, idx) => step.classList.toggle('is-active', idx === active));
  ...
  // ── 10. Jump-to-panel (rail clicks) ─────────────────────────────
  function jumpTo(i){
    recalcScroll();
    const top = (clamp(i, 0, N-1) / (N-1)) * maxScroll;
  ...
  // ── 11. Auto-hide hint after a while ────────────────────────────
  setTimeout(() => { if(hint) hint.classList.add('gone'); }, 8000);

  // ── 12. Debug hook for screenshot harness ───────────────────────
